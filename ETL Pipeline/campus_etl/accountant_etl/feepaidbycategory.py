...
def update_or_insert_data(rows, conn_dw):
    ...
    for row in rows:
        ...
        # Check if data for campus and category exists in target table
        cursor_dw.execute("SELECT COUNT(*) FROM FeePaidByCategory WHERE campus_name = ? AND category = ?", (campus_name, category))
        row_count = cursor_dw.fetchone()[0]

        if row_count > 0:
            # Data for campus and category exists, update fee_paid
            cursor_dw.execute("UPDATE FeePaidByCategory SET fee_paid = ? WHERE campus_name = ? AND category = ?", (fee_paid, campus_name, category))
        else:
