...
def update_or_insert_data(rows, conn_dw):
    ...
    cursor_dw.execute("Delete from Students")
    conn_dw.commit()
    for row in rows:
        degree_id = row[0]
        ...
        cursor_dw.execute("INSERT INTO Students (degree_id, total_students) VALUES (?, ?)", (degree_id, total_students))
...
LEFT JOIN
    FeeInstallments AS FI ON UF.challan_no = FI.challan_id 
	where FI.status = 1
GROUP BY
    U.degree_id
;
